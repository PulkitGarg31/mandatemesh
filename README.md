...
**Demo video (5 min):** https://youtu.be/5oTLv3iOoYc
...
|---|---|
| **Explainable** | Every Payment Link carries the intent / cart / payment mandate ids in its `notes`; the ledger records the rule-by-rule decision trail (`gate.decision`, `refund.decision`) for every authorization. `ledger receipt` exports one payment as Markdown with the cart, the purchase trail, the refund trail when there was a refund, and the chain head hash. |
| **Bounded** | Total cap, per-transaction cap, merchant allow-list, category list, currency and expiry are enforced by a pure function (`gate.py`: 20 ordered rules plus an error guard), not by a prompt. Cap breaches require a user-signed step-up token bound to that exact cart and total; that token waives the breached cap for **every** link of the chain for that one cart, not only the root's. A delegated agent is bounded twice: by its own sub-mandate and by every mandate above it. |
...
- `ledger replay` re-decides every `gate.decision` and `refund.decision` in the file and prints `N decisions replayed, N identical`, then a `chain: verified` / `chain: BROKEN at seq N` line, because reasoning integrity and record integrity are two different questions. Exit 0 when every recorded decision is what the gate recomputes *and* the chain verifies; exit 2 on the first divergence, on a decision whose inputs are not in the file (reported as unreplayable, never as a pass), on a broken chain even if every decision replayed identically, and if the path is missing. A ledger with no decisions in it prints `no gate decisions in this ledger` and exits 0.
...
| 2 | Broken or missing ledger (`ledger verify` fails, or the path is missing or not a file; `ledger replay` reports a broken chain too, and exits 2 for it), a replay divergence (`ledger replay` found a decision the gate does not recompute, or could not rebuild one), or a caught runtime error printed as `error: …` (missing `keys/`, unknown payment id in `receipt`, out-of-range `tamper` seq, a malformed `feed.json`). |
...
A valid step-up token covers both caps (R14 and R15) for that one cart, and it covers them for **every** link of a delegated chain, not only the root's: the user is approving this cart's total outright, so no link's cap is left to block it. `approved_total_paise` is the exact cart total shown in the prompt, so step-up is an explicit human override of the caps for exactly that cart, not a blanket raise. The step-up token is re-used if the gate re-runs for a retry.
...
R06, R12, R13 and R17 use the **leaf** mandate — the last link, whose bounds R19 has already proved to be a subset of every ancestor's. R14 and R15 are checked against **every** link, root to leaf: on a delegated chain the failing detail names the root-most link that breached the cap (so an over-spend does not look like only the junior agent's fault) and the passing detail names the tightest one, while an undelegated run has one cap to name and its detail simply says whether the cart is within it. R18 records its verdict before R19 runs, so a chain that is well-formed but too generous still leaves a passing `R18_DELEGATION_CHAIN` in the trail next to the R19 denial.
...
A **Sub-Mandate** is signed by the delegating *agent* (not the user), with its own caps, merchant allow-list, categories, currency and expiry. The gate walks the chain root-first and checks two things separately. R18 is the chain's shape: every link is signed by the agent the previous link authorized, points at that previous link as its parent, and appears once — a repeated `sub_id` is refused, and so is a chain longer than `MAX_DELEGATION_LINKS = 8`. That length check bounds the parse as well, so a padded chain is rejected on length before any signature is verified or any extra link is parsed. R19 is the arithmetic: both caps at most the parent's, merchants and categories subsets of the parent's, expiry no later than the parent's, and not already expired. Both checks run for every link, so a sub-mandate cannot quietly re-widen something an ancestor narrowed, however deep it sits. Revocation reaches the whole chain: R18 requires every link's delegator to be registered *and* active, so revoking one delegator in the registry invalidates that link and everything hanging off it, not just that agent's own proposals.
